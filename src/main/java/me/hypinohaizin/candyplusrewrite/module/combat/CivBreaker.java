...
public class CivBreaker extends Module {
    ...
    public CivBreaker() {
        ...
        targetType = register(new Setting<>("Target", CivBreaker.Target.Nearest));
    ...
    public void doCV() {
        ...
        try {
            if (!findMaterials()) {
                if (toggle.getValue()) { sendMessage("Cannot find materials! disabling..."); disable(); }
                return;
            }

            if (lockedTargetId == null) {
                CivBreaker.target = findTarget();
                if (isNull(CivBreaker.target)) {
                    if (toggle.getValue()) { sendMessage("Cannot find target! disabling..."); disable(); }
                    return;
                }
                if (FriendManager.isFriend(CivBreaker.target.getName())) {
                    reset();
                    return;
                }
                lockedTargetId = CivBreaker.target.getEntityId();
                lockTicks = LOCK_TICKS_DEFAULT;
            } else {
                Entity e = mc.world.getEntityByID(lockedTargetId);
                CivBreaker.target = (e instanceof EntityPlayer) ? (EntityPlayer) e : null;
                if (isNull(CivBreaker.target) || FriendManager.isFriend(CivBreaker.target.getName())) {
                    reset();
                    return;
                }
            }
            ...
            if (lockedBase == null) {
                if (!findSpace(CivBreaker.target)) {
                    if (toggle.getValue()) { sendMessage("Cannot find space! disabling..."); disable(); }
                    return;
                }
                lockedBase = base;
                lockTicks = LOCK_TICKS_DEFAULT;
            ...
            if (!builtTrap && BlockUtil.getBlock(base) == Blocks.AIR && blockTimer.passedX(preDelay.getValue())) {
                setItem(obbySlot);
            ...
            if (builtTrap && !placedCrystal && crystalTimer.passedX(crystalDelay.getValue())) {
                if (CrystalUtil.canPlaceCrystal(base)) {
                    ...
                    if (packetCrystal.getValue()) {
                        mc.player.connection.sendPacket(new CPacketPlayerTryUseItemOnBlock(base, EnumFacing.UP, hand, 0.5f, 1.0f, 0.5f));
                    } else {
                        mc.playerController.processRightClickBlock(mc.player, mc.world, base, EnumFacing.UP, new Vec3d(0.5, 1.0, 0.5), hand);
                    }
            ...
            if (placedCrystal && !brokeBlock && breakTimer.passedX(breakDelay.getValue())) {
                ...
                if (!CivBreaker.breaking) {
                    if (!instantBreak.getValue()) {
                        if (!noSwingBlock.getValue()) mc.player.swingArm(EnumHand.MAIN_HAND);
                        mc.player.connection.sendPacket(new CPacketPlayerDigging(CPacketPlayerDigging.Action.START_DESTROY_BLOCK, base, EnumFacing.DOWN));
                        mc.player.connection.sendPacket(new CPacketPlayerDigging(CPacketPlayerDigging.Action.STOP_DESTROY_BLOCK, base, EnumFacing.DOWN));
                    } else {
                        if (!noSwingBlock.getValue()) mc.player.swingArm(EnumHand.MAIN_HAND);
            ...
            if (brokeBlock && !attackedCrystal) {
                ...
                if (attackTimer.passedX(attackDelay.getValue())) {
                    final BlockPos plannedCrystalPos = base.add(0, 1, 0);
    ...
    private void tryPlaceCrystalImmediate() {
        ...
        if (!CrystalUtil.canPlaceCrystal(base)) return;
    ...
    public void onRender3D() {
        try {
            if (isNull(CivBreaker.target) || isNull(base)) return;
            RenderUtil3D.drawBox(base, 1.0, blockColor.getValue(), 63);
            if (outline.getValue()) RenderUtil3D.drawBoundingBox(base, 1.0, width.getValue(), blockColor.getValue());
        } catch (final Exception ex) {}
    }
    ...
    public boolean findSpace(final EntityPlayer player) {
        ...
        for (final BlockPos offset : offsets) {
            ...
            if ((BlockUtil.getBlock(basePos) == Blocks.OBSIDIAN || BlockUtil.getBlock(basePos) == Blocks.BEDROCK)
                ...
                if (CrystalUtil.canPlaceCrystal(basePos.up())) {
    ...
    public boolean findMaterials() {
        ...
        if (itemCheck(crystalSlot) && mc.player.getHeldItemOffhand().getItem() == Items.END_CRYSTAL) crystalSlot = 999;
